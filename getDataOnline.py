...
def start(tickers, day_stop):
   ...
   while True: #now_day < day_stop:
        ...
        last_data_time = datetime.datetime.strptime(nowTradeStats[0][1], "%H:%M:%S").time()
        if last_data_time != save_time or save_time is None:
                #Проверка на новый час
                prev_time = lastHourOrderStats[0][0] + " " + lastHourOrderStats[0][1]
                ...
                if time_diff.total_seconds() >= 3600:
                        ...
                        if lastHourOrderStats[0] == titles_orderStats:
                                lastHourOrderStats.insert(titles_orderStats)

                        lastHourTradeStats = pd.DataFrame((lastHourTradeStats + nowTradeStats), columns=titles_tradeStats)
                        lastHourOrderStats = pd.DataFrame((lastHourOrderStats + nowOrderStats), columns=titles_orderStats) 
                        ...
                        lastHourTradeStats:list = getDataTradeStats.getDataHourTradeStats(lastHourTradeStats, True, tickers)
                        lastHourOrderStats:list = getDataOrderStats.getDataHourOrderStats(lastHourOrderStats, True, tickers)  
