...
def getDateRange(date1, date2):
    ...
    dateList = [str((date1 + datetime.timedelta(days=x)).strftime('%Y-%m-%d'))
    ...
    for share in data:
        ...
        if tickers is None or ticker in tickers:
            if ticker not in hashtab:
                ...
                time_beg = datetime.datetime.strptime(
                    new_data[-1][1], "%H:%M:%S")
                ...
                # put_orders_b;
                new_data[index_of_share][3] = float(
                    new_data[index_of_share][3]) + float(share[3])

                # put_orders_s;
                new_data[index_of_share][4] = float(
                    new_data[index_of_share][4]) + float(share[4])

                # put_val_b;
                new_data[index_of_share][5] = float(
                    new_data[index_of_share][5]) + float(share[5])

                # put_val_s;
                new_data[index_of_share][6] = float(
                    new_data[index_of_share][6]) + float(share[6])

                # put_vol_b;
                new_data[index_of_share][7] = float(
                    new_data[index_of_share][7]) + float(share[7])

                # put_vol_s;
                new_data[index_of_share][8] = float(
                    new_data[index_of_share][8]) + float(share[8])
                ...
                # put_vol;
                new_data[index_of_share][11] = float(
                    new_data[index_of_share][11]) + float(share[11])

                # put_val;
                new_data[index_of_share][12] = float(
                    new_data[index_of_share][12]) + float(share[12])

                # put_orders;
                new_data[index_of_share][13] = float(
                    new_data[index_of_share][13]) + float(share[13])

                # cancel_orders_b;
                new_data[index_of_share][14] = float(
                    new_data[index_of_share][14]) + float(share[14])

                # cancel_orders_s;
                new_data[index_of_share][15] = float(
                    new_data[index_of_share][15]) + float(share[15])

                # cancel_val_b;
                new_data[index_of_share][16] = float(
                    new_data[index_of_share][16]) + float(share[16])

                # cancel_val_s;
                new_data[index_of_share][17] = float(
                    new_data[index_of_share][17]) + float(share[17])

                # cancel_vol_b;
                new_data[index_of_share][18] = float(
                    new_data[index_of_share][18]) + float(share[18])

                # cancel_vol_s;
                new_data[index_of_share][19] = float(
                    new_data[index_of_share][19]) + float(share[19])

                # cancel_vwap_b;cancel_vwap_s; средневзвешенная цена - вредный признак

                # cancel_vol;
                new_data[index_of_share][22] = float(
                    new_data[index_of_share][22]) + float(share[22])

                # cancel_val;
                new_data[index_of_share][23] = float(
                    new_data[index_of_share][23]) + float(share[23])

                # cancel_orders;
                new_data[index_of_share][24] = float(
                    new_data[index_of_share][24]) + float(share[24])

                # SYSTIME

                # создаём новую строку для следующего часа
                prev_time = new_data[index_of_share][0] + \
                    " " + new_data[index_of_share][1]
...
#     filenames = ['orderstats_2020.csv', 'orderstats_2021.csv',
#                  'orderstats_2022.csv', 'orderstats_2023.csv', 'orderstats_2023_rem.csv']
...
#     getDataTicker(None, datetime.date(2023, 11, 1),
#                   datetime.date(2023, 12, 8), filenames[-1])
